    ...
    def _provision_path(self, path: Path, wavelength: int):
        ...
        for i in range(len(path.node_list) - 1):
            ...
            self.topology[path.node_list[i]][path.node_list[i + 1]][
                "running_services"
            ].append(self.current_service.service_id)
            self._update_link_stats(path.node_list[i], path.node_list[i + 1])
        self.topology.graph["running_services"].append(self.current_service.service_id)
        self.current_service.wavelength = wavelength
